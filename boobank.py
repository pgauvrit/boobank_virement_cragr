...
class Boobank(ReplApplication):
    ...
    def do_transfer(self, line):
        ...
        self.logger.debug('from = '+id_from)
        for recipient in self.do('iter_transfer_recipients', account.id, backends=account.backend):
            #self.logger.debug('recipientid = '+recipient.id)
            self.logger.debug('recipientlabel = '+recipient.label)
            if id_to == recipient.label:
                self.logger.debug(recipient.id)
                id_to_orig = id_to
                id_to = str(recipient.id)+'@'+account.backend

        id_to, backend_name_to = self.parse_id(id_to)

        if account.backend != backend_name_to:
            print("Transfer between different backends is not implemented dd", file=self.stderr)
            return 4
        ...
        self.start_format()
        for transfer in self.do('transfer', id_from, id_to_orig, amount, reason, backends=account.backend):
