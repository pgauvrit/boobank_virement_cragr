...
from weboob.capabilities.bank import Account, Transfer, TransferError
...
class TransferPage(Page):
    def get_transfer_target_accounts(self):
        ...
        for select in self.document.xpath('//select'):
            if not select.attrib.get('id', '')=='compte-emmet' and not select.attrib.get('id', '')=='devise':
               ...
               for i in range(1,int(count)):
                    ...
                    for opttext in opttextraw:
                        ...
                        if not (s is None):
                            ...
                            j=j+1
                            account = Account_perso(target_account_value, s.group(2))
    ...
    def is_transfer_page(self):
        """
            ...
            transfer
        """
        link = self.document.xpath('//label[contains(text(), "Compte b")]')
        return bool(link)
...
class SavingsPage(_AccountsPage):
    ...
    def transfer_page_url(self):
        """
            ...
            current page is the saving page
        """
